...
class VariationalAutoencoder:
    def __init__(self, D, hidden_layer_sizes):
        ...
        # the encdoer's final layer output is unbounded
        ...
        # now build the cost (expected log likelihood, KL divergence)
        ...
        self.train_op = theano.function(
            inputs=[self.X], outputs=self.elbo, updates=updates
        )

        # returns a sample from p(x_new | X)
        self.posterior_predictive_sample = theano.function(
            inputs=[self.X], outputs=self.posterior_predictive,
        )
        ...
        self.transform = theano.function(
            inputs=[self.X], outputs=self.means
        )

        # returns a sample from p(x_new | z), from a given z
        self.prior_predictive_sample_with_input = theano.function(
            inputs=[Z_input], outputs=prior_predictive_probs_from_Z_input
        )
